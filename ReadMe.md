...
We implemented a Continuous Integration (CI) pipeline defined in `.github/workflows/security.yml`. This workflow triggers automatically on every `push` and `pull_request` targeting the `main` branch. If it fails, **the push/merge will be rejected** by github using github branch rules.
